...
<body class="relative h-screen w-full bg-cover bg-center font-sans overflow-hidden flex justify-end" style="background-image: url('images/IMG_8620.JPG');">
    ...
    <div class="relative z-10 w-full max-w-[420px] h-full bg-kdesigns-cream shadow-2xl flex flex-col border-l border-gray-300">
        ...
        <div class="bg-kdesigns-burgundy px-6 py-5 flex items-center justify-between flex-shrink-0">
            ...
            <a href="index.php" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition">
                    ...
                    <?php foreach ($user_orders as $order): ?>
                        <div class="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
                            <div class="p-4 flex gap-4 items-center">
                                <img src="<?= htmlspecialchars($order['image']); ?>" alt="Product" class="w-16 h-16 object-cover rounded-sm border border-gray-100 flex-shrink-0">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-2 mb-1">
                                        <h4 class="font-serif font-bold text-sm text-gray-900 truncate"><?= htmlspecialchars($order['product_name']); ?></h4>
                                        <span class="px-2 py-0.5 bg-amber-100 text-amber-800 font-bold text-[9px] tracking-wider uppercase rounded">
                                            <?= htmlspecialchars($order['status']); ?>
                                        </span>
                                    </div>
                                    <p class="font-serif font-bold text-sm text-gray-900 mb-1"><?= htmlspecialchars($order['price']); ?></p>
                                    <p class="text-[11px] text-gray-500"><?= htmlspecialchars($order['fulfillment']); ?> · <?= htmlspecialchars($order['date_needed']); ?> - <?= htmlspecialchars($order['time_needed']); ?></p>
                                </div>
                            </div>
                            <div class="px-4 py-2.5 bg-[#fbf9f5] border-t border-gray-100 flex items-center justify-between text-[11px] text-gray-500">
                                <span class="font-medium text-gray-700">Order #<?= htmlspecialchars($order['id']); ?></span>
                                <span>Placed <?= htmlspecialchars($order['placed_date']); ?></span>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="p-4 border-t border-kdesigns-inputBorder flex justify-end flex-shrink-0">
            <button class="w-8 h-8 rounded-full bg-gray-200 text-gray-700 flex items-center justify-center text-xs font-bold hover:bg-gray-300 transition">
                ?
            </button>
        </div>
    </div>
